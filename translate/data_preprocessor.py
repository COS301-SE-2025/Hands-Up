...
# IMPORTANT: This now points to the CSV generated by wlasl_parser.py for your 20 words
PROCESSED_DATA_CSV = 'wlasl_20_words_personal_processed.csv' # <-- CHANGED THIS LINE
...
def augment_sequence(sequence, max_rotation_deg, max_scale_factor, max_jitter_amount):
    ...
    # Generate random augmentation parameters for scaling
    # Move scale definition outside the inner loop to prevent UnboundLocalError
    # and ensure consistent scaling parameters for all parts within one frame
    scale = np.random.uniform(1.0 - max_scale_factor, 1.0 + max_scale_factor) # Defined once per augment call


    temp_sequence = []
    for frame_lms in augmented_sequence: # This is the augmented_sequence from rotation
        scale_per_frame = np.random.uniform(1.0 - max_scale_factor, 1.0 + max_scale_factor) # <-- Corrected UnboundLocalError here
        ...
        if pose_lms.size > 0 and not np.all(pose_lms[:,:3] == 0):
            ...
            pose_lms[:, :3] *= scale_per_frame # Apply per-frame scale
        ...
        if left_hand_lms.size > 0 and not np.all(left_hand_lms == 0):
            ...
            left_hand_lms *= scale_per_frame # Apply per-frame scale
        ...
        if right_hand_lms.size > 0 and not np.all(right_hand_lms == 0):
            ...
            right_hand_lms *= scale_per_frame # Apply per-frame scale
        ...
        if face_lms.size > 0 and not np.all(face_lms == 0):
            ...
            face_lms *= scale_per_frame # Apply per-frame scale
        processed_parts.append(face_lms.flatten())
        
        temp_sequence.append(np.concatenate(processed_parts))
    
    augmented_sequence = np.array(temp_sequence, dtype=np.float32) # This is the final augmented sequence after rotation and scaling per frame
...
if __name__ == "__main__":
    ...
    final_processed_df.to_csv('wlasl_20_words_personal_final_processed_data_augmented_seq90.csv', index=False) # <-- CHANGED OUTPUT CSV NAME
