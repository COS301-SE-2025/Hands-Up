...
# IMPORTANT: This should now point to the CSV generated by create_personal_dataset_csv.py (your 40 words)
PROCESSED_DATA_CSV = 'wlasl_40_words_personal_processed.csv' # Changed to 40 words
...
NUM_AUGMENTATIONS_PER_TRAIN_VIDEO = 5 # As decided for aggressive augmentation
AUG_MAX_ROTATION_DEG = 10
AUG_MAX_SCALE_FACTOR = 0.1
AUG_MAX_JITTER_AMOUNT = 0.003 # As decided for reduced jitter

MAX_VIDEOS_FOR_TEST = None # Process the full subset
...
def augment_sequence(sequence, max_rotation_deg, max_scale_factor, max_jitter_amount):
    ...
    # Generate random augmentation parameters ONCE per sequence augmentation
    angle_rad = np.deg2rad(np.random.uniform(-max_rotation_deg, max_rotation_deg))
    cos_val = np.cos(angle_rad)
    sin_val = np.sin(angle_rad)
    scale = np.random.uniform(1.0 - max_scale_factor, 1.0 + max_scale_factor) # Scale generated here

    temp_sequence = []
    for frame_lms in augmented_sequence: # Iterate through each frame in the sequence
        processed_parts = []
        current_idx = 0

        # Apply rotation and scaling to each part
        # Pose landmarks (33 points × 4 coords each)
        ...
        if pose_lms.size > 0 and not np.all(pose_lms[:,:3] == 0):
            ...
            pose_lms[:, :3] *= scale # Apply the single 'scale' generated once per sequence
        processed_parts.append(pose_lms.flatten())

        # Left hand landmarks (21 points × 3 coords each)
        ...
        if left_hand_lms.size > 0 and not np.all(left_hand_lms == 0):
            ...
            left_hand_lms *= scale # Apply the single 'scale' generated once per sequence
        processed_parts.append(left_hand_lms.flatten())

        # Right hand landmarks (21 points × 3 coords each)
        ...
        if right_hand_lms.size > 0 and not np.all(right_hand_lms == 0):
            ...
            right_hand_lms *= scale # Apply the single 'scale' generated once per sequence
        processed_parts.append(right_hand_lms.flatten())

        # Face landmarks (468 points × 3 coords each)
        ...
        if face_lms.size > 0 and not np.all(face_lms == 0):
            ...
            face_lms *= scale # Apply the single 'scale' generated once per sequence
        processed_parts.append(face_lms.flatten())

        temp_sequence.append(np.concatenate(processed_parts))

    # Convert to numpy array after rotation and scaling for all frames
    augmented_sequence = np.array(temp_sequence, dtype=np.float32)

    # 3. Add random jitter (noise)
...
if __name__ == "__main__":
    ...
    if not os.path.exists(PROCESSED_DATA_CSV):
        print(f"Error: Processed data CSV not found at {PROCESSED_DATA_CSV}. Please run create_personal_dataset_csv.py first to create it (for fixed list).")
        exit()
    ...
    final_processed_df.to_csv('wlasl_40_words_personal_final_processed_data_augmented_seq90.csv', index=False) # Changed to 40 words
    print("\nFinal processed data metadata saved to 'wlasl_40_words_personal_final_processed_data_augmented_seq90.csv'") # Updated print message
