...
import pandas as pd
import numpy as np
from sklearn.model_selection import train_test_split # Not used in this specific version due to pre-split CSV
...
PROCESSED_DATA_CSV = 'wlasl_20_words_personal_final_processed_data_augmented_seq90.csv'
PROCESSED_SEQUENCES_DIR = 'processed_sequences' # Not directly used in this script, but good for context
...
DROPOUT_RATE = 0.5 # <-- THIS MUST BE 0.5
LEARNING_RATE = 0.0005 # <-- THIS MUST BE 0.0005
BATCH_SIZE = 32
EPOCHS = 100 # Maximum epochs, EarlyStopping will stop it earlier
...
def build_lstm_model(input_shape, num_classes, lstm_units, dense_units, dropout_rate, learning_rate):
    model = Sequential([
        ...
        Dense(dense_units, activation='relu', kernel_regularizer=l2(0.001)), # Added L2 Regularization
        BatchNormalization(), # Added Batch Normalization
        Dropout(dropout_rate),

        Dense(num_classes, activation='softmax', kernel_regularizer=l2(0.001)) # Added L2 Regularization
    ])
...
if __name__ == "__main__":
    ...
    # Assuming the 'split' column already exists in your CSV for train/val/test
    ...
    # Using .keras extension for saving models (recommended by Keras 3+)
