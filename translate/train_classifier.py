...
# IMPORTANT: This now points to the CSV generated by data_preprocessor.py for your 30 words
PROCESSED_DATA_CSV = 'wlasl_30_words_final_processed_data_augmented_seq90.csv' # <-- CHANGED TO 30 WORDS CSV
PROCESSED_SEQUENCES_DIR = 'processed_sequences' 
MODEL_SAVE_DIR = 'saved_models' 

SEQUENCE_LENGTH = 90 # Should be correct from duration analysis
EXPECTED_COORDS_PER_FRAME = 1662 

# --- Model Hyperparameters (ADJUSTED FOR 30 WORDS) ---
LSTM_UNITS = 128 # <-- CHANGED: Increased from 96 (for more capacity with 30 words)
DENSE_UNITS = 192 # <-- CHANGED: Increased from 128 (for more capacity with 30 words)
DROPOUT_RATE = 0.55 # <-- CHANGED: Slightly increased from 0.5 (more aggressive regularization for larger model)
LEARNING_RATE = 0.0005 # Keep this (from rebalancing)
BATCH_SIZE = 32 
EPOCHS = 100 
...
if __name__ == "__main__":
    ...
    if not os.path.exists(PROCESSED_DATA_CSV):
        ...
        print("Please ensure data_preprocessor.py has been run to create this file (for your 30-word list, sequence 90).") 
        exit()
    ...
    print(f"Number of unique classes: {num_classes}") # Should be 30
    ...
    early_stopping = EarlyStopping(monitor='val_accuracy', patience=30, restore_best_weights=True) # <-- CHANGED: Patience to 30
    checkpoint_filepath = os.path.join(MODEL_SAVE_DIR, 'best_sign_classifier_model_30_words_seq90.keras') # <-- CHANGED MODEL SAVE FILENAME
    model_checkpoint = ModelCheckpoint(checkpoint_filepath, monitor='val_accuracy', save_best_only=True, verbose=1)
    reduce_lr = ReduceLROnPlateau(monitor='val_accuracy', factor=0.5, patience=15, min_lr=0.00001, verbose=1) # <-- CHANGED: Patience to 15
