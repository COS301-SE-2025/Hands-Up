...
"""
# Preprocess_Data.py (Example - actual implementation needed)
import numpy as np
import os
import random
...
def load_data(data_path, actions, sequence_length=30):
    sequences, labels = [], []
    ...
    for action in actions:
        ...
        if not os.path.exists(action_path):
            print(f"Warning: Action directory not found: {action_path}")
            continue
        ...
        for sequence_file in os.listdir(action_path):
            if sequence_file.endswith('.npy'):
                ...
                try:
                    # Load a single sequence (e.g., (30, 1662) shape)
                    # Assuming each .npy file contains a single sequence of keypoints
                    keypoints_sequence = np.load(full_path)
                    
                    # Ensure sequence length matches, or pad/truncate
                    if keypoints_sequence.shape[0] != sequence_length:
                        # Simple padding/truncation strategy
                        if keypoints_sequence.shape[0] > sequence_length:
                            keypoints_sequence = keypoints_sequence[:sequence_length]
                        else:
                            padding = np.zeros((sequence_length - keypoints_sequence.shape[0], keypoints_sequence.shape[1]))
    ...
    if not sequences:
        print("No sequences loaded. Check data_path and action names.")
