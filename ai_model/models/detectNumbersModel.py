...
for i, item in enumerate(dataDictTrain['data']):
    if isinstance(item, (np.ndarray, list)) and len(item) == 63:
...
cleanedTestData = []
cleanedTestLabels = []
for i, item in enumerate(dataDictTest['data']):
    if isinstance(item, (np.ndarray, list)) and len(item) == 63:
        cleanedTestData.append(np.array(item, dtype=np.float32))
        cleanedTestLabels.append(dataDictTest['labels'][i])

xTest = np.array(cleanedTestData)
yTestRaw = np.array(cleanedTestLabels)

xTrain = xTrain.reshape(-1, 63, 1)
xTest = xTest.reshape(-1, 63, 1)
...
dataDictTrain2 = pickle.load(open('../processed_data/numFineTuneData.pickle', 'rb'))
...
xTrain2 = xTrain2.reshape(-1, 63, 1)
...
model = tf.keras.models.Sequential([
    tf.keras.layers.Conv1D(32, kernel_size=3, activation='relu', input_shape=(63, 1)),
...
model.compile(optimizer=tf.keras.optimizers.Adam(1e-5),loss=tfa.losses.SigmoidFocalCrossEntropy(),metrics=['accuracy'])
