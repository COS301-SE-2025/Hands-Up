...
export function PhilSigns({ landmarks, replay }) {
    ...
    useEffect(() => {
        ...
        for (const key in landmarks) {
            if (key.startsWith('mixamorig')) {
                const newKey = key.replace('mixamorig', 'mixamorig9');
                remapped[newKey] = landmarks[key];
            }
        }
        renamedLandmarks.current = remapped;
    }, [landmarks]);
    ...
    useFrame(() => {
        ...
        for (const boneName in renamedLandmarks.current) {
        const bone = bones.current[boneName];
        if (!bone) continue;
        
        const keyframes = renamedLandmarks.current[boneName].keyframes;
    ...
    return <primitive object={scene} scale={2} position={[0.1, -1.75, 0]} />;
}
