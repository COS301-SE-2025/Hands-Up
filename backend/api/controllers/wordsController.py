...
TIME_BETWEEN_WORDS = 1.0
SENTENCE_CLEAR_DURATION = 8.0
...
def calculate_motion(current_lms, previous_lms):
    if previous_lms is None or np.all(previous_lms == 0):
        return 0.0
    pose_change = np.linalg.norm(current_lms[0:33*4:4] - previous_lms[0:33*4:4])
    hands_change = np.linalg.norm(current_lms[33*4:] - previous_lms[33*4:])
    return pose_change + hands_change
...
def run_realtime_translator():
    ...
    if not os.path.exists(PROCESSED_DATA_CSV):
        print(f"Error: CSV not found at {PROCESSED_DATA_CSV}.")
        return
    ...
    while cap.isOpened():
        ...
        if results.pose_landmarks:
            pose_lms_flat = [coord for lm in results.pose_landmarks.landmark for coord in [lm.x, lm.y, lm.z, lm.visibility]]
        ...
        if results.left_hand_landmarks:
            lh_lms_flat = [coord for lm in results.left_hand_landmarks.landmark for coord in [lm.x, lm.y, lm.z]]
        ...
        if results.right_hand_landmarks:
            rh_lms_flat = [coord for lm in results.right_hand_landmarks.landmark for coord in [lm.x, lm.y, lm.z]]
            current_frame_raw_landmarks_flat[current_idx : current_idx + len(rh_lms_flat)] = rh_lms_flat
        current_idx += NUM_HAND_COORDS_SINGLE

        if results.face_landmarks:
            face_lms_flat = [coord for lm in results.face_landmarks.landmark for coord in [lm.x, lm.y, lm.z]]
        ...
        if current_state == STATE_PREDICTING:
            if recorded_raw_landmarks_buffer:
        ...
        cv2.putText(frame, display_status_text, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
        cv2.putText(frame, sentence_text, (10, 70), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
        cv2.putText(frame, display_confidence_text, (10, 110), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 165, 255), 2)
